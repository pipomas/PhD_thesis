...
Um erkennen zu können, ob der Zusammenhang zwischen den beiden Aufgabenparametern (Asymptote und Steigung) oder der Zusammenhang zwischen der Asymptote respektive der Steigung mit dem \gls{zwert} des \gls{bist} durch diejenigen \glspl{vp} verzerrt wurde, bei welchen eine Beschreibung der Daten mit einem exponentiellen Modell nicht angebracht war, wurden diese Zusammenhänge in Abhängigkeit des \gls{rmse} bestimmt. 
Dafür wurde für jeden \gls{rmse} zwischen 1 und 70 eine Teilstichprobe mit \glspl{vp} gebildet, welche den gewählten \gls{rmse} (\gls{rmse}-Grenzwert) nicht überschritten haben. Die erste Teilstichprobe ($n=3$) bestand also aus \glspl{vp}, welche einen \gls{rmse} von nicht grösser 1 aufwiesen. Die zweite Teilstichprobe ($n=11$) setzte sich aus \glspl{vp} zusammen, welche einen \gls{rmse} von nicht grösser 2 aufwiesen. Die dritte Teilstichprobe ($n=48$) beinhaltete \glspl{vp}, welche einen \gls{rmse} von nicht grösser 4 aufwiesen (usw.). Dieses Vorgehen wurde solange weitergeführt, bis bei einem \gls{rmse}-Grenzwert von 70 die Gesamtstichprobe ($N=177$) zur Bestimmung der Zusammenhänge verwendet wurde.
...
Die Analyse hat ergeben, dass die Asymptote und die Steigung ab einem \gls{rmse}-Grenzwert von $1.4$ ms stark negativ miteinander zusammenhingen ($r=-.98$ bis $r=-.57$, alle $p\textnormal{s}<.05$). Eine visuelle Inspektion des Verlaufs deutete darauf hin, dass der \gls{rmse}-Grenzwert einen negativen Einfluss auf die Höhe des Zusammenhangs ausübte (siehe \autoref{fig:spatial_suppression_rmse_cutoff_asymtote_slope}).

\begin{figure}[t]
	\centering
	%	\captionsetup{font = small}
	\begin{adjustbox}{width=1\textwidth}
		\input{tikzDevice/spatial_suppression_rmse_cutoff_asymptote_slope.tex}
	\end{adjustbox}
	\caption[RMSE einfluss]{Einfluss des \gls{rmse}-Grenzwerts auf den Zusammenhang ($r$) zwischen den beiden aus der \gls{ssauf} mit einer exponentiellen Regression abgeleiteten Asymptote und Steigung. Die durchgezogene Linie kennzeichnet den Verlauf des Zusammenhangs. Der graue Bereich beschreibt das $95\,\%$-Konfidenzintervall.}
	\label{fig:spatial_suppression_rmse_cutoff_asymtote_slope}
\end{figure}
...
Während der Zusammenhang über einen grossen Teil des tieferen \gls{rmse}-Grenzwertbereichs nicht signifikant war, unterschritt der Zusammenhang zwischen $8.6$ und $9.5$ ms ($r = -.17$, $p = .049$), zwischen $26.1$ und $35.7$ ms ($r = -.15$ bis $r = -.16$, alle $p\textnormal{s} < .048$) und ab $36.5$ ms ($r = -.16$ bis $r=-.18$, alle $p\textnormal{s} < .03$) die Signifikanzgrenze. Eine visuelle Inspektion des Verlaufs liess keine Aussage darüber zu, ob der  \gls{rmse}-Grenzwert einen positiven oder negativen Einfluss auf die Höhe des Zusammenhangs ausübte.
...
Die Steigung und der \gls{zwert} des \gls{bist} korrelierten unabhängig vom \gls{rmse}-Grenzwert nicht signifkant miteinander ($r=-.16$ bis $r=.62$, alle $p\textnormal{s}>.08$) . (siehe \autoref{fig:spatial_suppression_rmse_cutoff}b).
