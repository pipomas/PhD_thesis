...
Abschliessend zur zweiten Fragestellung kann festgehalten werden, dass die aus der exponentiellen Regression abgeleiteten Aufgabenparameter in Bezug auf psychometrische Intelligenz keinen prädiktiven Wert aufwiesen. 
...
Alle konfirmatorischen Faktorenanalysen wurden mit der Satorra-Bentler Maximum-Likelihood Schätzmethode \citep{Satorra1994} berechnet, weil diese bei nicht-normal\-ver\-teilten, intervallskalierten Daten anderen Schätzmethoden überlegen ist \citep{Curran1996, Finney2006}.
...
Abschliessend zur dritten Fragestellung kann festgehalten werden, dass sich zwischen der \gls{ssauf} und psychometrischer Intelligenz auf latenter Ebene ein geringer bis mittlerer negativer Zusammenhang zeigte. Eine schnellere Erkennung der Bewegungsrichtung in der \gls{ssauf} war also mit besserer kognitiver Leistung im \gls{bist} verbunden (und v. v.).
...
Mit der vierten Fragestellung sollte versucht werden, die \gls{ssauf} mit einem \gls{flm} zu beschreiben und die zwei aus der Aufgabe abgeleiteten latenten Variablen mit dem \gls{gfaktor} in Verbindung zu bringen.


\subsection{Fixed-Links-Messmodell der \gls{ssauf} \label{subsec:flss}}


Weil die Aufgabe bisher noch nie mit einem \gls{flm} beschrieben wurde, wurden unterschiedliche Modelle getestet und miteinander verglichen. Bei allen berechneten Modellen wurden zwei voneinander unabhängige latente Variablen angenommen: 
Die erste latente Variable beinhaltete aufgabenrelevante Prozesse, deren Einflüsse sich über die vier Bedingungen hinweg nicht geändert haben. In den Messmodellen wurde dieser gleichbleibende Einfluss hergestellt, indem die unstandardisierten Faktorladungen aller manifesten Variablen auf den Wert 1 fixiert wurden. Weil mit diesen Fixierungen der Einfluss der in der latenten Variable abgebildeten Prozesse über die vier Bedingungen konstant gehalten wird, wird diese latente Variable im Folgenden \textit{konstante} latente Variable genannt. 
Die zweite latente Variable beinhaltete aufgabenrelevante Prozesse, die durch die vier Bedingungen systematisch manipuliert wurden. Der unterschiedlich starke Einfluss der in der latenten Variable abgebildeten Prozesse auf die Bedingungen der \gls{ssauf} wurde durch sich unterscheidende unstandardisierte Faktorladungen hergestellt. Diese latente Variable wird im Folgenden \textit{dynamische} latente Variable genannt. Um die in der Diskussion folgende Interpretation der beiden latenten Variablen zu vereinfachen, wurde die konstante latente Variable in allen Messmodellen unabhängig von der dynamischen latenten Variable gehalten und ist im Rahmen der Anwendung von \gls{flm}en üblich \citep[z.B.][]{Leue2012, Schweizer2007, Wang2015}.
Alle Modell-Fits der in den folgenden Unterabschnitten berichteten \gls{flm}e sind zur Übersicht in Tabelle \ref{tab:fixed-links_measurement_models_spatial_suppression} aufgeführt.

Das erste berechnete \gls{flm} (Modell 3) berücksichtige das Ergebnis der exponentiellen Regression (siehe Abschnitt \ref{sec:2Fragestellung}), welches auf manifester Ebene eine Steigung von $b=0.103$ ergeben hat. Die unstandardisierten Faktorladungen der dynamischen latenten Variable wurden deshalb mit diesem Parameter [$y=e^{0.102x},\,x\in\{1, 2, 3, 4\}$] gebildet. Modell 3 bildete die empirischen Varianzen und Kovarianzen der \gls{ssauf} nicht gut ab. Der \gls{cst} war hochsignifikant und der \gls{cfi}, der \gls{rmsea} und das \gls{srmr} lagen nicht im akzeptablen Bereich.

Modell 4 berücksichtigte die Tatsache, dass die Mustergrösse über die vier Bedingungen linear zunahm ($1.8^{\circ}$, $3.6^{\circ}$, $5.4^{\circ}$, $7.2^{\circ}$).
Die unstandardisierten Faktorladungen der dynamischen latenten Variable in Modell 4 wurden deshalb linear ansteigend [$y=x,\,x\in\{1, 2, 3, 4\}$] gewählt. Modell 4 bildete die empirischen Varianzen und Kovarianzen der \gls{ssauf} ebenfalls nicht gut ab. Der \gls{cst} war hochsignifikant und der \gls{rmsea} und das \gls{srmr} lagen nicht im akzeptablen Bereich 
...
Nach diesen zwei Modellen, welche klare Annahmen über den Verlauf der Faktorladungen der dynamischen latenten Variable machten, wurden als nächstes explorativ Verläufe von Faktorladungen gesucht, welche die empirischen Daten bestmöglich beschreiben. 
Die Faktorladungen der dynamische latente Variable von Modell 5 wurden mit einer exponentiellen Funktion [$y=2^x,\,x\in\{1, 2, 3, 4\}$] bestimmt. Dieses Modell konnte nicht interpretiert wurde, weil die Fehlervarianz der $7.2^{\circ}$-Bedingung negativ geschätzt wurde. 

In Modell 6 wiesen die Faktorladungen der dynamischen latenten Variable einen logarithmischen Verlauf [$y=\log_{e}x,\,x\in\{1, 2, 3, 4\}$] auf. Das Modell bildete die empirischen Varianzen und Kovarianzen der \gls{ssauf} nicht adäquat ab. Der \gls{cst} war signifikant und der \gls{rmsea} sowie das \gls{srmr} lagen nicht im akzeptablen Bereich. 

Die Faktorladungen der dynamischen latenten Variable von Modell 7 wurden mit einer quadratischen Funktion [$y=x^2,\,x\in\{1, 2, 3, 4\}$] bestimmt. Dieses Modell bildete die empirischen Varianzen und Kovarianzen der \gls{ssauf} akzeptabel ab. Der \gls{cst} war nicht signifikant und der \gls{cfi} lag im akzeptablen Bereich. Einzig der \gls{rmsea} und das \gls{srmr} deuteten auf eine nicht gute Passung des Modells hin.

Als letztes wurden die Faktorladungen der dynamischen latenten Variable in Modell 8 (siehe Abbildung \ref{fig:fixed-links_measurement_model_spatial_suppression}) linear ansteigend [$y=x,\,x\in\{0, 1, 2, 3\}$] fixiert, die erste Bedingung im Gegensatz zu Modell 4 aber auf 0 gesetzt. Dieses Modell bildete die empirischen Varianzen und Kovarianzen der \gls{ssauf} im Vergleich mit den Modellen 3 bis 7 am besten ab. Der \gls{cst} war nicht signifikant und der \gls{cfi} und der \gls{rmsea} lagen im akzeptablen Bereich. Einzig das \gls{srmr} deutete auf eine nicht gute Passung des Modells hin. 
...
Der relative Anteil dieser beiden Varianzen an der in den manifesten Variablen erklärten Varianz liess sich aufgrund der in konfirmatorischen Faktorenanalysen gegebenen multiplikativen Verknüpfung von Faktorladungen und Varianzen nicht direkt ermitteln. Um die Varianzen miteinander vergleichen zu können, wurden der Einfluss der Faktorladungen auf die Varianzen mit der Methode von \citet{Schweizer2011a} kontrolliert. Diese Skalierung der Varianzen hat ergeben, dass die konstante latente Variable 72\,\% und die dynamische latente Variable 28\,\% der in den manifesten Variablen gemeinsamen Varianz band.
Weil Modell 8 die empirischen Daten am besten abbildete, wurde es für alle weiteren Berechnungen verwendet. 
...
Abschnitt \ref{sec:3Fragestellung} hat gezeigt, dass das herkömmliche Strukturgleichungsmodell einen schlechten Modell-Fit ergab. 

Modell 8 wurde mit dem \gls{gfaktor} aus dem \gls{bist} in Verbindung gebracht (Modell 9; siehe Abbildung \ref{fig:ssflg}).
Das Modell bildete die empirischen Daten gut ab [$\upchi^2(14)=19.06$, $p=.16$, $\textnormal{CFI}=.99$, $\textnormal{RMSEA}~=~.05$, $\textnormal{SRMR}~=~.09$]. Die konstante latente Variable sagte den \gls{gfaktor} mit $\upbeta~=~-.25$ ($p~=~.02$) und die dynamische latente Variable den \gls{gfaktor} nicht signifikant? mit $\upbeta~=~-.08$ ($p~=~.43$) vorher.

Gleiche Varianzaufklärung wie die herkömmliche Analyse, aber bessere Modellbeschreibung!
Ähnlich wie auf manifester Ebene, das Ergebnisbild.

\begin{figure}[htb]
	\centering
	\includegraphics[width=\textwidth]{../jpg/ssflg}
	\caption[CFA Platzhalter Test]{Modell 9: Latenter Zusammenhang zwischen dem \gls{flm} (Modell 8) der \gls{ssauf} und dem \gls{gfaktor} aus dem \gls{bist}. Eingezeichnet sind die standardisierten Koeffizienten.\\
	$^{*}~p~<~.05$. $^{**}~p~<~.01$. $^{***}~p~<~.001$.
