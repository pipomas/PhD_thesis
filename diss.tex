...
% Setup for acronym package ------------------------------------------------------
%\usepackage[printonlyused]{acronym}				% use acronym package if 'glossaries' is not in effect
...
\usepackage{float}					% see package documentation
...
%\numberwithin{equation}{section}				% equations starten bei 1.1 1.2, 2.1 2.2 etc
%\numberwithin{figure}{section} 				% das glieche bei figure
...
\setcounter{lofdepth}{2}			% include minicaptions
...
% Define command "myindenpar" for a paragraph ----------------------------------------
...
\usepackage[tocbib, natbibapa, nosectionbib]{apacite}	% define options
\bibliographystyle{apacite-mod}								% for style 'Author, N. (Year). Title, ...'
...
\AtBeginDocument{\renewcommand{\BRetrievedFrom}{Verfügbar unter\ }} % removes  "Zugriff auf"
...
%			pdfsubject={Dissertation},
			...
			pdfkeywords={LaTeX, R, Intelligence, Spatial Suppression}
			}
...
% Glossary entries --------------------------------------------------------
%\newacronym{ssa}{}{Spatial-Suppression-Ansatz}
%\newglossaryentry{ssa}	{name = {Spatial-Suppression-Ansatz}, description={test}}
...
%\newacronym{rzn}{RZn}{Reaktionszeiten}
...
%\renewcommand{\familydefault}{\sfdefault}		% <- what does this do o_O ?
\begin{document}
\layout								% prints layout page at the beginning of the document
\frontmatter						% triggers roman numerals 
%\maketitle							% not needed when creating a custom title page
...
\begin{itemize}
	...
	\item Trotz Kürze sollte über die zu prüfenden psychologischen H	ypothesen, die Methode, die Ergebnisse
...
%\addcontentsline{toc}{chapter}{Abkürzungen} 	% add acr to toc
%%\begin{acronym}
...
\subsection{Versuchsablauf \label{sub:}}
...
\section{Lage-, Streuungs- und Zusammenhangsmasse \label{sec:DeskriptiveAnalyse}}
...
Deskriptiven Angaben zur Aufgabe sind in Tabelle \ref{tab:SSdescriptives} zu finden und die Zusammenhänge zwischen den Bedingungen sind in Tabelle \ref{tab:SScorr} abgetragen.
...
%
	...
	\caption[Spatial-Suppression-Streudiagramm]{Streudiagramme der vier Mustergrössen der \gls{ssauf}. Die horizontale Linie kennzeichnet jeweils den Mittelwert der Schwellenschätzungen innerhalb einer Bedingung. Vp = Versuchsperson.}
	\label{fig:SSscatter}
\end{figure}
...
\begin{table}[htbp]
	...
	\centering
	\captionsetup{labelsep = none}
	\caption[Deskriptive Angaben zur ]{\newline  \textit{Deskriptive Angaben zur \gls{ssauf} und Kennwerte zur Verteilung der Daten} \vspace{.2cm}}
	...
	\begin{threeparttable}
		...
		\begin{tabular}{
			...
			\cline{2-5}
			Bedingung 		&	\textit{M}	&	\textit{SD}	&	{Min}	&	Max 	&	\textnormal{Schräge}	&	\textnormal{Kurtosis}\\% &S-W \textit{p}-Wert\\
			\hline
			\(1.8^{\circ}\)	&	85			&	27			&	31		&	161		&	0.46					&	-0.14		\\%			& 		$.008$	\\
			\(3.6^{\circ}\)	&	94			&	33			&	37		&	282		&	1.73					&	6.75		\\%			& 		$<.001$	\\
			\(5.4^{\circ}\)	&	116			&	52			&	45		&	422		&	2.84					&	11.39		\\%			& 		$<.001$	\\
			\(7.2^{\circ}\)	&	149			&	85			&	61		&	705		&	3.23				x	&	14.01		\\%			& 		$<.001$	\\
			\hline
		\end{tabular}%
		%}
					\begin{tablenotes}[flushleft]
						\footnotesize				% font size
						\setlength\labelsep{0pt}	% no indent on second line
						\item \textit{Anmerkungen.} Min~=~Minimum, Max~=~Maximum.
					\end{tablenotes}%
	\end{threeparttable}%
	%}
\end{table}
...
\begin{table}[htbp]
	...
	\centering
	\captionsetup{labelsep = none}
	\caption[Korrelationeeeeeen]{\newline  \textit{Spearman's Rangkorrelationen der vier Bedingungen der \gls{ssauf}} \vspace{.2cm}}
	\label{tab:SScorr}
	%	\resizebox{\columnwidth}{!}{%	
	\begin{threeparttable}
...
Die mittleren \glspl{rz} sind in Abbildung \ref{fig:Hickscatter} zu sehen. Deskriptive Angaben zur Aufgabe sind in Tabelle \ref{tab:Hickdescriptives} zu finden und die Zusammenhänge zwischen den Bedingungen sind in Tabelle \ref{tab:Hickcorr} abgetragen.
...
\end{tikzpicture}
	...
	\caption[Hick-Aufgabe-Streudiagramm]{Streudiagramme der vier Bedingungen der \gls{ha}. Die horizontale Linie kennzeichnet jeweils den Mittelwert der Reaktionszeiten innerhalb einer Bedingung. Vp = Versuchsperson.}
...
\begin{table}[htbp]
	...
	\centering
	\captionsetup{labelsep = none}
	\caption[Deskriptive Angaben zur ]{\newline  \textit{Deskriptive Angaben zur \gls{ha} und Kennwerte zur Verteilung der Daten} \vspace{.2cm}}
...
%				S[table-format = 1.2, add-integer-zero=false]
			...
			Bedingung	& 	{\textit{M}}&{\textit{SD}}	&	{Min}	&	{Max} 	&	{\textnormal{Schräge}}	&	{\textnormal{Kurtosis}}\\% &S-W \textit{p}-Wert\\
			\hline
			0-bit		&	240			&	29			&	188		&	394		&	1.58					&	4.96	\\%				& 		$.008$			\\
			1-bit		&	296			&	32			&	234		&	416		&	0.93					&	1.31	\\%				& 		$<.001$			\\
			2-bit		&	377			&	54			&	280		&	590		&	0.87					&	0.99	\\%				& 		$<.001$			\\
			2.58-bit	&	438			&	67			&	315		&	650		&	0.82					&	0.39	\\%				& 		$<.001$			\\
			\hline
		\end{tabular}%
		%}
		\begin{tablenotes}[flushleft]
...
\begin{table}[htbp]
	...
	\centering
	\captionsetup{labelsep = none}
	\caption[Korrelationen Hick]{\newline  \textit{Spearman's Rangkorrelationen der vier Bedingungen der \gls{ha}} \vspace{.2cm}}
	\label{tab:Hickcorr}
	%	\resizebox{\columnwidth}{!}{%	
	\begin{threeparttable}
		\sisetup{table-space-text-post = $^{***}$}
...
\begin{table}[htbp]
	...
	\centering
	\captionsetup{labelsep = none}
	\caption[Deskriptive Angaben zur ]{\newline  \textit{Deskriptive Angaben zum \gls{bist} und Kennwerte zur Verteilung der Daten} \vspace{.2cm}}
...
%				S[table-format = < 0.3, add-integer-zero=false]
			...
			\hline
			& 	\multicolumn{4}{c}{in Rohwerten}	&		&				\\
			
			\cline{2-5}
			Subtest &  {\textit{M}}	& {\textit{SD}}	&	{Min}	&	{Max} 	& \textnormal{Schräge}	& \textnormal{Kurtosis}\\% & {S-W  \textit{p}-Wert}\\
			\hline
		...
		\begin{tablenotes}[flushleft]
			\footnotesize				% font size
			\setlength\labelsep{0pt}	% no indent on second line
			\item \textit{Anmerkungen.} Siehe Tabelle \ref{tab:BIS} für eine Beschreibung der Subtests. Min~=~Minimum, Max~=~Maximum.
		\end{tablenotes}%
	\end{threeparttable}%
	%}
\end{table}
...
%				S[table-format = 1.2, add-integer-zero=false]
			...
			\cline{2-5}
			Parameter	& 	{\textit{M}}	&{\textit{SD}}	&	{Min}	&	{Max} 	&	{\textnormal{Schräge}}	&	{\textnormal{Kurtosis}}\\% &S-W \textit{p}-Wert\\
			\hline
...
\begin{figure}[htb]
	...
	\caption[CFA Platzhalter Test]{Modell 9: Latenter Zusammenhang zwischen dem \gls{flm} der \gls{ssauf} und dem \gls{gfaktor} aus dem \gls{bist}. Eingezeichnet sind die standardisierten Koeffizienten. $^{*}~p~<~.05$, $^{**}~p~<~.01$, $^{***}~p~<~.001$.
...
%	\flushleft
	\centering
	\captionsetup{labelsep = none}
	\caption[Korrelationen all]{\newline  \textit{Spearman's Rangkorrelationen zwischen den Bedingungen der \gls{ssauf}, der \gls{ha} und dem \textit{z}-Wert des \gls{bist}} \vspace{.2cm}}
...
%	\resizebox{1.5\columnwidth}{!}{%	
	...
	\end{tabular}%
		...
		\begin{tablenotes}[flushleft]
			\footnotesize				% font size
			\setlength\labelsep{0pt}	% no indent on second line
			\item \textit{Anmerkungen.} {$^{*}$}$p<.05$, {$^{**}$}$p<.01$, {$^{***}$}$p<.001$ (zweiseitig).
		\end{tablenotes}%
	\end{threeparttable}%
%	}
\end{table}
...
\begin{table}[htbp] % see http://tex.stackexchange.com/questions/247921/different-column-widths-under-a-multicolumn-prevent-appropriate-centering
	...
	\centering
	\captionsetup{labelsep = none}
	\caption[Korrelationen all]{\newline  \textit{Spearman's Rangkorrelationen zwischen regressionsanalytisch abgeleiteten Parametern der \gls{ssauf}, der \gls{ha} und dem \textit{z}-Wert des \gls{bist}} \vspace{.2cm}}
	\label{tab:ababz}
	%	\resizebox{1.5\columnwidth}{!}{%	
	\begin{threeparttable}
		\sisetup{table-space-text-post = $^{***}$}
		...
		\begin{tablenotes}[flushleft]
			\footnotesize				% font size
			\setlength\labelsep{0pt}	% no indent on second line
			\item \textit{Anmerkungen.} \textit{a}~=~Asymptote bei der \gls{ssauf}, Achsenabschnitt bei der \gls{ha}. \textit{b}~=~Steigung. {$^{*}$}$p<.05$, {$^{**}$}$p<.01$, {$^{***}$}$p<.001$ (zweiseitig).
		\end{tablenotes}%
	\end{threeparttable}%
	%	}
\end{table}
...
\bibliography{../references/PhilippLibrary}		% provide .bib file
...
Die Daten von drei \glspl{vp} wurden von der Analyse ausgeschlossen, weil sie verglichen mit den restlichen \glspl{vp} auf der Stimulusgrösse von \(1.6^{\circ}\) eine $\log_{10}$-Schwel\-len\-schätz\-ung hatten, die über das dreifache der \gls{sd} der Gesamtstichprobe betrug. Diese drei \glspl{vp} wurden nicht zur Grundpopulation gezählt und vor der Analyse entfernt.
