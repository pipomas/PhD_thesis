...
Mit der zweiten Fragestellung sollte geprüft werden, ob die mit einer exponentiellen Regression abgeleiteten Aufgabenparameter benutzt werden können, um psychometrische Intelligenz vorherzusagen.



Für jede \gls{vp} wurden die vier Schwellenschätzungen mit einer exponentiellen Regression der Form $y=a \times e^{bx}$ vorhergesagt (siehe Abbildung \ref{fig:spatial_suppression_exponential_model}). 

\begin{figure}[htbp]
	\centering
	\resizebox{\textwidth}{!}{
		\input{tikzDevice/spatial_suppression_exponential_model.tex}}
	\caption[Exponentielle Regression]{Exponentieller Einfluss der Mustergrösse auf die Wahrnehmungsleistung in der \gls{ssauf}. Eingezeichnet sind die Mittelwerte $\pm$ Standardfehler der Mittelwerte. $y=70 \times e^{0.102x}$. }
	\label{fig:spatial_suppression_exponential_model}
\end{figure}

Deskriptive Angaben zu den daraus resultierenden Parametern der Asymptote $a$ und der Steigung $b$ sind in Tabelle \ref{tab:AsymSlope} zu finden. Über alle \glspl{vp} gemittelt betrug $R^2=.80$, wobei aber eine beträchtliche individuelle Variabilität (\gls{sd}~$=.25$, Min~=~$.005$, Max~=~$.999$) bestand.
...
\begin{figure}[htbp]
	...
	\caption[Dichtefunktion des RMSE]{Dichtefunktion des Root Mean Square Error (RMSE).  Alle Datenpunkte sind auf der \textit{x}-Achse mit vertikalen Strichen markiert.}
