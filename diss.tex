...
\begin{table}[htbp]
	...
	\captionsetup{labelsep = none}
	\caption[Effektstärken zwischen den Bed SS]{\newline \textit{Effektstärken (Cohens \textit{d}) der Mittelwertunterschiede in der \gls{ssauf}} \vspace{.2cm}}
	...
	\begin{threeparttable}
		\begin{tabular}{
			...
			1	&	\(1.8^{\circ}\)	&			&			&			&			\\
			2	&	\(3.6^{\circ}\)	&	0.28	&			&			&			\\
			3	&	\(5.4^{\circ}\)	&	0.82	&	0.56	&			&			\\
			4	&	\(7.2^{\circ}\)	&	1.35	&	1.10	&	0.57	&	\hphantom{1.00}		\\
...
Produkt-Moment-Korrelationen zwischen den vier Bedingungen der \gls{ssauf} deuteten ausnahmslos auf starke Zusammenhänge hin (siehe Tabelle \ref{trololol}).

Der \gls{si} wies einen Mittelwert $\pm$ Standardabweichung von $0.222\,\pm\,0.160$ auf (Minimum $= -0.185$, Maximum $= 0.886$). 
Die Verteilung des \gls{si} (siehe Abbildung \ref{fig:SIhist}) hatte eine Schiefe von $0.91$ und eine Kurtosis von $1.80$ auf, welche ein Shapiro-Wilk-Test als eine signifikante Abweichung von der Normalverteilung erkannte ($p<.001$).
