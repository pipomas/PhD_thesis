...
Die Schwellenschätzungen aller \glspl{vp} der \gls{ssauf} sind in Abbildung \ref{fig:SSscatter} zu sehen. Die dazugehörigen deskriptiven Angaben sind in Tabelle \ref{tab:SSdescriptives} abgetragen. 
Um für die Analyse der \gls{ssauf} auf manifester Ebene adäquate Auswertungsverfahren zu bestimmen, wurden die vier Bedingungen auf Normalverteilung getestet. 
Shapiro-Wilk-Tests haben ergeben, dass die Schwellenschätzungen der vier Bedingungen nicht normalverteilt waren (alle $p\textnormal{s}<.01$, siehe Tabelle \ref{tab:SSdescriptives}). 
...
Um zu prüfen, ob die Mustergrösse einen signifikanten Einfluss auf die Wahrnehmungsleistung der \glspl{vp} ausübte, wurde ein Friedman-Test durchgeführt. Der Globaltest wies darauf hin, dass sich die Bedingungen signifikant voneinander unterschieden, $\upchi^2(3)=343.88$, $p<.001$. Die im Anschluss an den Friedman-Test gerechnete Post-hoc-Tests \citep{Galili2010, Hollander2014} haben gezeigt, dass sich von den (durch die vier Bedingungen ergebenen) sechs Einzelvergleichen nur die \(1.8^{\circ}\)- und \(3.6^{\circ}\)-Bedingung nicht signifikant voneinander unterschieden ($p=.12$). Die restlichen fünf Einzelvergleiche waren mit $p<.001$ alle statistisch signifikant.
...
Spearmans Rangkorrelationen ($r_{s}$) zwischen den vier Bedingungen der \gls{ssauf} wiesen ausnahmslos auf starke Zusammenhänge hin. Relativ betrachtet bestand der tiefste Zusammenhang zwischen der \(1.8^{\circ}\)- und der \(7.2^{\circ}\)-Bedingung ($r_{s}~=~.54$, $p~<~.001$) und der höchste Zusammenhang zwischen der \(5.4^{\circ}\)- und der \(7.2^{\circ}\)-Bedingung ($r_{s}~=~.88$, $p~<~.001$). Tabelle \ref{tab:shzmanifest} gibt einen Überblick über alle Interkorrelationen der vier Bedingungen in der \gls{ssauf}.
...
%
	...
	\caption[Spatial-Suppression-Streudiagramm]{Streudiagramme der Schwellenschätzungen in der \gls{ssauf}. Die horizontale Linie kennzeichnet jeweils den Mittelwert der Schwellenschätzungen innerhalb einer Bedingung (vgl. Tabelle \ref{tab:SSdescriptives}). Vp = Versuchsperson.}
	\label{fig:SSscatter}
\end{figure}
...
\begin{table}[t]
	...
	\centering
	\captionsetup{labelsep = none}
	\caption[Deskriptive Angaben zur Schwellenbestimmung in der \gls{ssauf}]{\newline  \textit{Deskriptive Angaben zur Schwellenbestimmung der \gls{ssauf} in Millisekunden (Mittelwert, Standardabweichung, Minimum, Maximum) und Kennwerte zur Verteilung der Daten} \vspace{.2cm}}
...
Die \glspl{rz} aller \glspl{vp} der \gls{ha} sind in Abbildung \ref{fig:Hickscatter} zu sehen. Die dazugehörigen deskriptiven Angaben sind in Tabelle \ref{tab:Hickdescriptives} abgetragen.


Wie bei der Analyse der \gls{ssauf} mussten als erstes die datengerechten Auswertungsverfahren bestimmen werden. Dafür wurden die \glspl{rz} der vier Bedingungen mit Shapiro-Wilk-Tests auf Normalverteilung getestet. Die Tests wiesen darauf hin, dass die Verteilungen nicht einer Normalverteilung entsprachen (alle $p\textnormal{s}<.001$, siehe Tabelle \ref{tab:Hickdescriptives}). Infolgedessen wurden wie bei der Analyse der \gls{ssauf} verteilungsfreie Auswertungsverfahren eingesetzt.
