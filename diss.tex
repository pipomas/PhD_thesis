...
Analog zur zweiten Fragestellung

\subsubsection{Bedingungsebene}
...
\begin{table}[htbp]
	\centering
	\captionsetup{labelsep = none}
	\caption[Vorhersage ]{\newline  \textit{Multiple Regression zur Vorhersage des \gls{zwert}s des \gls{bist} durch die Bedingungen der \gls{ha} (Modell 10) und durch die Bedingungen der \gls{ha} und der \gls{ssauf} (Modell 11)} \vspace{.2cm}}
	...
	\begin{threeparttable}
		\begin{tabular}{
			...
			{Variable}	&	{$b$}	&	{\textit{SE}(\textit{b})}	&	{$\upbeta$}	&	{$p$}	&	 &{$F$}		&	 {$R^2$}	& {$\Delta F$} & {$\Delta R^2$}   \\
