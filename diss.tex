...
Der \gls{si} wies einen Mittelwert $\pm$ Standardabweichung von $0.221\,\pm\,0.160$ (Minimum $= -0.185$, Maximum $= 0.886$) auf. Die positive Schiefe ($0.91$) und Kurtosis ($1.80$) deutete im Vergleich mit der Normalverteilung auf eine linkssteile, spitze Verteilung hin. Ein Shapiro-Wilk-Test zeigte dann mit $p<.001$ auch an, dass der \gls{si} nicht normalverteilt war (siehe Abbildung \ref{fig:SIhist}).
...
Bevor ausgewählte Zusammenhänge zwischen den Aufgaben in den folgenden Abschnitten mit den Fragestellungen abgearbeitet werden, ist der Vollständigkeit halber in Tabelle \ref{tab:correlations_manifest} die  Korrelationsmatrix abgebildet. 
...
Der \gls{si} wies eine negative Korrelation mit der \(1.8^{\circ}\)-Bedingung auf ($r_{s}=-.33$, $p<.001$) und korrelierte positiv mit der \(7.2^{\circ}\)-Bedingung ($r_{s}=.53$, $p<.001$). 
Diese Zusammenhänge können als Hinweis dafür gesehen werden, dass der \gls{si} als Differenz zwischen der $\log_{10}$-Schwellenschätzung für die Mustergrösse \(7.2^{\circ}\) und der $\log_{10}$-Schwellenschätzung für die Mustergrösse \(1.8^{\circ}\) korrekt gebildet wurde (siehe Abschnitt \ref{subsec:Prozedur}).
Weiter korrelierte einzig die 0-bit-Bedingung der \gls{ha} mit der \(3.6^{\circ}\)- und der \(5.4^{\circ}\)-Bedingung der \gls{ssauf} signifikant ($r_{s}=.19$ respektive $r_{s}=.16$, $p\textnormal{s}<.05$), während alle restlichen Zusammenhänge zwischen den beiden Aufgaben so gering ausfielen, dass sie die gewählte Irrtumswahrscheinlichkeit von $5\,\%$ nicht unterschritten. 
Die Bedingungen der \gls{ha} korrelierten alle signifikant negativ mit dem \gls{zwert} des \gls{bist} ($r_{s}=-.15$ bis $-.32$), während die Bedingungen der \gls{ssauf} nicht signifikant mit dem \gls{zwert} zusammenhingen ($r_{s}=.04$ bis $-.14$).
